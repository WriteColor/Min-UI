...
// exact color maps from sphere.html
...
export default function Orb({ state, volume, theme }: OrbProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Habilitar el cambio de paleta dinámico
      ...
      if (lerpT < 1) {
        ...
        const s = 0.028;
        const lerpHex = (a: string, b: string) => {
          const ca = new THREE.Color(a);
          const cb = new THREE.Color(b);
          ca.lerp(cb, s);
          return "#" + ca.getHexString();
        };
        lerpColors.PRI = lerpHex(lerpColors.PRI, targetPalette.PRI);
        lerpColors.PRI_DIM = lerpHex(lerpColors.PRI_DIM, targetPalette.PRI_DIM);
        lerpColors.ALT = lerpHex(lerpColors.ALT, targetPalette.ALT);
  ...
  return <div ref={containerRef} className="w-full h-full" style={{ backgroundColor: "#000", position: "relative", overflow: "hidden" }} />;
}
